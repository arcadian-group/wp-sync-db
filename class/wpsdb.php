...
class WPSDB extends WPSDB_Base {
	...
	function get_upload_info($type = 'path') {
		/*
			* Let developers define their own path for export files
			*
		...
		*/
		...
		if (!empty($upload_info) && isset($upload_info[$type])) {
		...
		$full_path = $upload_dir['basedir'] . DS . $upload_dir_name;

		if (!file_exists($full_path)) {
			if (!$this->create_upload_dir($full_path)) {
				return $upload_info[$type];
			}
		}
	...
	private function create_upload_dir($path) {
		if (false === @mkdir($path, 0755)) {
			return false;
		}

		$index_file = $path . DS . 'index.php';
		$index_content = "<?php\n// Silence is golden\n?>";
		if (false === @file_put_contents($index_file, $index_content)) {
			return false;
		}
	...
	function output_diagnostic_info() {
		...
		echo 'MySQL: ';
		echo esc_html($wpdb->use_mysqli ? mysqli_get_server_info($wpdb->dbh) : '');
		echo "\r\n";
	...
	function get_table_row_count() {
		...
		foreach ($results as $result) {
			if (isset($result['table_name']) && isset($result['TABLE_ROWS'])) {
